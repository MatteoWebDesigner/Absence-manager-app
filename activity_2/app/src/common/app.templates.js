angular.module('AbsenceManager').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('/src/absence-calendar/views/absence-calendar.html',
    ...
    "\t\t\t\t'active-pm' : day.pm\r" +
    "\n" +
    "\t\t\t}\"></div><div class=pm></div><div class=number>{{ day.number }}</div><div class=absent-user-number>{{ day.user.length }} <i class=\"fa fa-user\"></i></div></li></ul><ul class=legend><li><i class=dot></i> No absence</li><li class=user-absence><i class=dot></i> User absence</li><li class=others-absence><i class=dot></i> Others absence</li></ul></div>"
  );
